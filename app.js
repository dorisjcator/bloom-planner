...
function rN(){
  ...
  for(var i=0;i<TB.length;i++){
    h+='<button class="ntb'+(Q.pg===TB[i].k?' on':'')+'" onclick="go(\''+TB[i].k+'\')">'+TB[i].l+'</button>';
  }
...
window.go=function(k){
  Q.pg=k;
  for(var i=0;i<TB.length;i++){
    var el=document.getElementById('pg-'+TB[i].k);
    if(el) el.className=TB[i].k===k?'pg on':'pg';
  }
  rN();D();
};
...
function mkCard(e){
  ...
  if(e.co) h+='<span class="cst">$'+parseFloat(e.co).toFixed(2)+'</span>';
...
function mkToolbar(){
  var h='<div class="tb">';
  h+='<input class="si" placeholder="\uD83D\uDD0D Search..." value="'+es(Q.s)+'" oninput="Q.s=this.value;D()">';
  h+='<select class="so" onchange="Q.o=this.value;D()">';
  h+='<option value="da"'+(Q.o==='da'?' selected':'')+'>Date \u2191</option>';
  h+='<option value="dd"'+(Q.o==='dd'?' selected':'')+'>Date \u2193</option>';
  h+='<option value="cd"'+(Q.o==='cd'?' selected':'')+'>Cost \u2193</option>';
  h+='<option value="nm"'+(Q.o==='nm'?' selected':'')+'>Name</option>';
  h+='</select></div>';
  return h;
}
...
function rBud(){
  ...
  var tEntrySpent=me.reduce(function(s,e){return s+(parseFloat(e.co)||0)},0);
  ...
  h+='<div class="zbb">';
  h+='<div style="font-size:11px;opacity:.7;margin-bottom:2px">INCOME \u2212 PLANNED = ZERO</div>';
  h+='<div style="font-size:26px;font-weight:800">$'+tInc.toFixed(0)+' \u2212 $'+tPlan.toFixed(0)+' = <span style="color:'+zc+'">$'+left.toFixed(0)+'</span></div>';
  h+='<div style="margin-top:4px;font-size:12px;font-weight:700">'+zm+'</div>';
  h+='</div>';
  ...
  // Debt overview if any debt lines exist
  ...
  if(debtLines.length){
    ...
    h+='<div><div style="font-size:10px;color:#dc2626;font-weight:700">\u26D3\uFE0F TOTAL DEBT</div><div style="font-size:24px;font-weight:800;color:#dc2626">$'+totalDebt.toFixed(2)+'</div><div style="font-size:10px;color:#b89aac">'+debtLines.length+' accounts</div></div>';
  ...
  h+='<div><label>SOURCE</label><input id="biL" placeholder="Paycheck..."></div>';
  h+='<div><label>AMOUNT</label><input type="number" id="biA" placeholder="3500"></div>';
  h+='<button onclick="aI()">+ Add</button>';
  h+='</div></div>';
  ...
  h+='<button onclick="cpL()" style="padding:5px 10px;border-radius:7px;border:1px solid #f5d0e6;background:#fff;color:#b45992;font-size:10px;font-weight:700;cursor:pointer">\uD83D\uDCCB Copy Last Month</button>';
  h+='</div>';
  ...
  for(var j=0;j<BC.length;j++){
    ...
    h+='<span class="arr'+(isCol?'':' u')+'">\u25BC</span>';
    h+='</div></div>';

    if(catPlan>0){
      var pCol=catPct>100?'#e11d48':catPct>80?'#d97706':bc.c;
      h+='<div class="bpbar"><div class="bpfill" style="width:'+catPct+'%;background:'+pCol+'"></div></div>';
    }
    ...
    if(bc.hasBal&&catLines.length){
      ...
      h+='<div style="text-align:right"><span style="font-size:9px;color:#059669;font-weight:700">Paid This Month</span><strong style="display:block;font-size:16px;color:#059669">$'+dPaid.toFixed(2)+'</strong></div>';
      h+='</div>';
    }
...
window.tPd=function(id){
  for(var i=0;i<B.length;i++){
    if(B[i].id===id){
      ...
      D();tt(B[i].pd?'\u2705 Marked paid':'\u21A9\uFE0F Unmarked');
      return;
    }
  }
};
